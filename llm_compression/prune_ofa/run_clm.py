...
These changes were first implemented by Zafrir et al. and adapted to Causal Language Modelling
Original repo: https://github.com/IntelLabs/Model-Compression-Research-Package/
"""
...
from llm_compression.api_utils import (
...
from llm_compression.kronecker import HfKroneckerCallback
from llm_compression.lowrank import HfSVDCallback
...
logger.addFilter(lambda s: not re.match(".*use_cache=False.*", s.getMessage()))
...
class DistillationArguments:
    distill: bool = field(default=False, metadata={"help": "Perform model distill"})
    teacher_name_or_path: Optional[str] = field(
        default=None, metadata={"help": "path to distillation teacher"}
    )
    ...
    cross_model_distillation: Optional[str] = field(
        default=None,
        metadata={"help": "Whether to use distilbert or tinybert distillation method"},
    )
...
class DataTrainingArguments:
    ...
    # list in run_mlm; why?
    # dataset_name_config: Optional[List[str]] = field(
    #         default=None, metadata={"help": "The name:config of the dataset to use (via the datasets library)."}
    # )
    ...
    data_process_type: str = field(
        default="concatenate",
        metadata={
            "help": f"The preprocess method to use for data preprocessing. "
        },
    )
...
def main():
    ...
    parser.add_argument("--kronecker", action="store_true", default=False)
    ...
    if distill_args.distill:
        ...
        logger.info(
            "*** Initializing teacher from {} ***".format(
        ...
        if distill_args.cross_model_distillation is not None:
            if distill_args.cross_model_distillation == "distilbert":
                hidden_alpha = defaultdict(lambda: 0.0)
                hidden_alpha[model.config.num_hidden_layers] = (
                    1
                    - distill_args.cross_entropy_alpha
                    - distill_args.knowledge_distillation_alpha
                )
                similarity_loss = "cosine_embedding"
            # if distill_args.cross_model_distillation == "tinybert":
            else:
                attention_alpha = defaultdict(lambda: 1.0)
                hidden_alpha = defaultdict(lambda: 1.0)
    ...
    if pruning_args.do_prune and pruning_args.lr_rewind:
        ...
        transformers.optimization.TYPE_TO_SCHEDULER_FUNCTION[
            transformers.SchedulerType("linear")
        ] = lr_sched_fn

    callbacks = [pruning_callback]
    if pruning_args.kronecker:
        callbacks.append(HfKroneckerCallback())
    if pruning_args.svd:
        callbacks.append(HfSVDCallback())
    ...
    if training_args.do_train:
        ...
        try:
            torch.save(
                ...
                os.path.join(training_args.output_dir, "args.bin"),
            )
        except:
