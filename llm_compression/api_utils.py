...
from .prune_ofa.pruning.schedulers import *
from . import distiller
from .prune_ofa.pruning import registry
...
def add_pruning_arguments_to_parser(parser):
    ...
    parser.add_argument(
        "--do_prune", action="store_true", help="Perform pruning when training a model"
    )
    parser.add_argument(
        "--pruning_config", type=str, default="", help="Path to a pruning config"
    )
    parser.add_argument(
        "--pruning_override",
        type=str,
        nargs="*",
        action=ConcatenateStringAction,
        default="",
        help="JSON string to override pruning configuration file",
    )
    return parser
...
def get_linear_rewinding_schedule_with_warmup(
    optimizer,
    num_warmup_steps,
    num_training_steps,
    prune_start_step,
    ...
    def pruning_lr_lambda(current_step: int):
        ...
        if current_step >= prune_start_step and current_step < prune_end_step:
            rewind = (
                (current_step - prune_start_step) // rewind_interval * rewind_interval
            )
            return max(
                0.0,
                float(train_wo_pruning_steps - current_step + rewind)
                / float(max(1, train_wo_pruning_steps - num_warmup_steps)),
            )
        return max(
            0.0,
...
try:
    ...
    class HFTrainerPruningCallback(TrainerCallback):
        ...
        def on_init_end(self, args, state, control, model=None, **kwargs):
            ...
            if self.args.do_prune and args.do_train:
                assert (
                    kwargs.pop("optimizer") is None
                ), "Optimizer needs to be initialized after pruning initialization"
        ...
        def on_train_end(self, args, state, control, model=None, **kwargs):
            if self.args.do_prune:
                ...
                torch.save(
                    state_dict, os.path.join(args.output_dir, "raw_pytorch_model.bin")
                )
                self.scheduler.remove_pruning()
                with open(
                    os.path.join(args.output_dir, "pruning_config.json"), "w"
                ) as file:
    ...
    class HFTeacherWrapper(distiller.TeacherWrapper):
        ...
        def __init__(self, *args, **kwargs):
            ...
            super().__init__(*args, **kwargs)
            self.logit_names = (
                logit_names if isinstance(logit_names, list) else [logit_names]
            )
            if self.weight is None:
                self.weight = torch.ones(len(self.logit_names)) / len(self.logit_names)
            self._input = None
        ...
        def forward(self, **inputs):
            ...
            ), "Cross model hidden states distillation loss is used but model doesn't output hidden_states"
            assert self.attention_alpha is None or teacher_inputs_.get(
                "output_attentions", False
            ), "Cross model attention states distillation loss is used but model doesn't output attentions"
            teacher_inputs = {}
        ...
        def compute_distill_loss_callback(self, student_outputs, teacher_outputs):
            ...
            if self.ce_alpha != 0:
                ...
                for logit_name, weight in zip(self.logit_names, self.weight):
                    ...
                    if (
                        len(teacher_logit.size()) > 2
                        and teacher_logit.size()[-1] == vocab_size
                    ):
                        teacher_logit = self._masked_outputs(teacher_logit, vocab_size)
                        student_logit = self._masked_outputs(student_logit, vocab_size)
                        assert teacher_logit.size() == student_logit.size()
                    ce_loss += weight * self.compute_cross_entropy_loss(
            ...
            for key, alpha_dict in zip(
                ["hidden_states", "attentions"],
                [self.hidden_alpha, self.attention_alpha],
            ):
        ...
        *,
        ...
        def forward(student, *args, **kwargs):
            ...
            if student.training:
                ...
                loss += student._teacher.compute_distill_loss(student_output)
